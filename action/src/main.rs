...
pub mod actions {
    ...
    pub fn get_input(name: impl AsRef<str>) -> Result<String, InputError> {
    ...
    pub fn get_multiline_input<'a>(name: impl AsRef<str>) -> Result<Vec<String>, InputError> {
    ...
    macro_rules! warning {
        ($($arg:tt)*) => {{
            $crate::actions::issue_level(
                $crate::actions::LogLevel::Warning,
                format!($($arg)*),
                None,
            );
        }};
...
async fn run() -> eyre::Result<()> {
    ...
    let path: PathBuf = actions::get_input("path")
        .map(PathBuf::from)
        .or_else(|_| std::env::current_dir())
        .wrap_err("path is not specified")?;

    let registry_token = actions::get_input("registry-token").ok();

    let dry_run = actions::get_input("dry-run")
        .ok()
        .map(parse_bool)
    ...
    let publish_delay = actions::get_input("publish-delay")
        .ok()
        .map(parse_duration)
        .map_or(Ok(None), |v| v.map(Some))
        .wrap_err("invalid value for publish-delay")?;

    let no_verify = actions::get_input("no-verify")
        .ok()
        .map(parse_bool)
        .map_or(Ok(None), |v| v.map(Some))
    ...
    let resolve_versions = actions::get_input("resolve-versions")
        .ok()
        .map(parse_bool)
        .map_or(Ok(None), |v| v.map(Some))
    ...
    actions::warning!("include: {:?}", actions::get_input("include"));
    actions::warning!("exclude: {:?}", actions::get_input("exclude"));
